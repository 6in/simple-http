...
let
  ...
  pcreDownloadLink = "https://downloads.sourceforge.net/pcre/" &
      pcreArchiveFile
  ...
  libreSslDownloadLink = "https://ftp.openbsd.org/pub/OpenBSD/LibreSSL/" &
      libreSslArchiveFile
  libreSslInstallDir = (thisDir() / "libressl/") & libreSslVersion
  libreSslConfigureCmd = ["./configure", "--disable-shared",
      "--prefix=" & libreSslInstallDir]
  ...
  openSslDownloadLink = "https://www.openssl.org/source/" &
      openSslArchiveFile
  ...
  openSslConfigureCmd = ["./Configure", openSslSeedConfigOsCompiler,
      "no-shared", "no-zlib", "no-async", "-fPIC",
      "-DOPENSSL_NO_SECURE_MEMORY", "--prefix=" & openSslInstallDir]
...
proc dollar[T](s: T): string =
  result = $s
proc mapconcat[T](s: openArray[T]; sep = " ";
    op: proc(x: T): string = dollar): string =
  ...
  if (not existsFile(libreSslLibFile)) or (not existsFile(libreCryptoLibFile)):
    ...
    withDir libreSslSourceDir:
      ...
      putEnv("CC",
                  ...
                  "musl-gcc -static -idirafter /usr/include/ -idirafter /usr/include/x86_64-linux-gnu/")
...
task musl, "Builds an optimized static binary using musl":
  ...
  for f in nimFiles:
    let
      ...
      binFile = thisDir() / "bin" / baseName # Save the binary in the same dir as the nim file
